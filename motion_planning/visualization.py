...
trajectory = np.load('data/output/test_whole_out_11_new_new.npy')
# trajectory = np.load('data/whole_astar_trajectory/T311.npy')
trajectory = trajectory[1:, :]
obstacles = np.load('data/whole_multi_trajectory/T11.npy')
# desired_indices = np.arange(2, whole_trajectory.shape[0], 3)
# trajectory = whole_trajectory[desired_indices]
triangle_meshes = obstacles[:, 2: 3 + trajectory.shape[0] // 3, :, :]  # Replace this with your actual triangle meshes
fig, ax = plt.subplots()
ax.set_xlim([-1, 1])
ax.set_ylim([-1, 1])
scat_agent = ax.scatter([], [], color='blue', label='Agent')
...
ani.save('data/output/test_whole_11_new_new.gif', writer=writer)
plt.show()
...
plt.savefig('data/output/test_whole_11_new_new.png')
# Show the plot
plt.show()
